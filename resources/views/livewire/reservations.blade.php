<div>
    ...
    <div class="flex justify-end w-full text-left">
        ...
        >
            {{($onlyFuture) ? __('All') : __('Only future')}}
        </x-button>
    </div>
    @endif
            ...
            >
                <div class="flex rounded-sm items-center justify-between p-2 bg-blue-800 whitespace-nowrap">
                    <p class="text-gray-200 mr-10">
                        {{__('Reservation') . ' '. $reservation->from->format($dateFormat)}},
                        {{__('seats') .' '. $reservation->tables_sum_number_of_seats}}.
                    </p>
                    <span class="flex justify-end items-center">
                        ...
                        <x-delete-button
                            wire:click="delete({{$reservation}})"
                            wire:confirm="{{__('Are you sure you want to delete reservation?')}}"
                        >
                            {{__('Delete')}}
                        </x-delete-button>
                    </span>
                ...
                >
                    @foreach ($reservation->tables as $table)
                        <div
                            ...
                            wire:key="{{$table->id}}"
                        >
                            {{$table->name}} ({{__('seats').' '.$table->number_of_seats}})
                        </div>
                    @endforeach
                </div>
            </div>
