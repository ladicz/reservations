...
The application keeps the UI layer focused on presentation and form state:

- Livewire components handle interactive screens and validation.
- Services contain reservation, table availability, and user-creation logic.
- Eloquent models define persistence and relationships.
- A policy limits reservation cancellation to the reservation owner.

Reservation creation runs inside a database transaction. Selected table rows are locked before checking conflicts, preventing concurrent requests from booking the same table.
...
Start the application containers, generate the application key, and create demo data:
...
- A table cannot be booked by overlapping reservations.
- A reservation must contain at least one existing, unique table ID.
